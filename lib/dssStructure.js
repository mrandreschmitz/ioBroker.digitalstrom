...
class DSSStructure {
        ...
        this.dssQueue.queueUpdateOutputValue(
            ...
            (err, value) => {
                ...
                if (outputType === 'brightness' && typeof dev.applyNativeLightValue === 'function') {
                    dev.applyNativeLightValue(value);
    ...
    applyChannelScaleValue(dev, outputType, value) {
        ...
        this.setStateSafe(stateId, rounded);
    ...
    createDevice(dev, callback) {
        ...
        if (dev.hwInfo.startsWith('SW-') || dev.hwInfo.startsWith('IC SW-')) {
            ...
            if (dev.outputMode && dev.outputChannels && dev.outputChannels.length) {
                ...
                // A scene changes these outputs as well. The bundled status request
                // coalesces a whole room into ONE read, so the values follow the scene
                // immediately instead of waiting for the next reconciliation. Without
                // the sync there is no burst-free way for vDC hardware - every device
                // would need its own named read - so the values wait there.
                if (
                    this.adapter.config.initializeOutputValues &&
                    ...
                    this.dss.on(dev.dSUID, data => {
                        ...
                        // Without the sync only a DEVICE scene reads back: it reaches
                        // exactly this dSUID, so it costs one request. A zone or
                        // apartment scene is fanned out to every device of the room -
                        // there the values wait for the next start instead.
    ...
    createLightDevice(dev, devId, callback) {
        ...
        dev.applyNativeLightValue = value => setLightValue('brightness', value);
