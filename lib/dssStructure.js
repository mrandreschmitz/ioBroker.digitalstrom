...
class DSSStructure {
    constructor(options) {
        ...
        // Client der neuen API, null wenn die Option aus ist. Wird ausschliesslich fuer die
        // Zaehler benutzt - alles andere laeuft weiter ueber die klassische API.
        this.smartHome = options.smartHome || null;
        // Ein Fehlschlag wird einmal gemeldet, danach nur noch als debug: faellt die neue
        // API dauerhaft aus, soll das Log nicht alle 100 Sekunden dieselbe Zeile bekommen
        this.smartHomeMeterFailed = false;
    ...
    reportUnmappedStates() {
        ...
        const unmapped = this.propertyStates
     ...
     *
     * @param {(errorCount: number, total: number) => void} [callback]
     * @returns {Promise<boolean>} false, wenn der Aufrufer auf die alte API zurueckfallen soll
     */
    async updateMeterDataViaSmartHome(callback) {
        if (!this.smartHome) {
            return false;
        }
        let values;
        try {
            ...
            const message = `Meter values could not be read via the Smart Home API (${errorMessage(err)}), using the classic API`;
            if (this.smartHomeMeterFailed) {
                this.adapter.log.debug(message);
            } else {
                this.smartHomeMeterFailed = true;
        ...
        for (const entry of values) {
            byId.set(entry.id, entry.attributes && entry.attributes.value);
        }

        let total = 0;
        let missing = 0;
        for (const circuit of this.apartmentCircuits || []) {
        ...
        if (total && missing === total) {
            ...
            this.adapter.log.warn('The Smart Home API returned no value for any known circuit, using the classic API');
            return false;
        }
        if (missing) {
            this.adapter.log.debug(`${missing} of ${total} circuits are missing in the Smart Home API answer`);
        }
        callback && callback(missing, total);
        return true;
    }

    updateMeterData(callback) {
        if (this.smartHome) {
            // Bei jedem Fehlschlag wird unten auf den klassischen Weg zurueckgefallen
            this.updateMeterDataViaSmartHome(callback).then(
                done => {
                    if (!done) {
                        this.updateMeterDataViaClassicApi(callback);
                    }
                },
                err => {
    ...
    updateMeterDataViaClassicApi(callback) {
        let updateCounter = 0;
        let totalCounter = 0;
        let errorCounter = 0;
        this.apartmentCircuits.forEach(circuit => {
            ...
            this.dssQueue.pushQueryQueue(
                ...
                (err, res) => {
                    if (!err && res && res.ok && res.result) {
                        this.setStateSafe(`devices.${circuit.dSUID}.PowerConsumption`, res.result.consumption);
                    } else {
                        errorCounter++;
                    }
                    !--updateCounter && callback && callback(errorCounter, totalCounter);
                },
            );
            ...
            this.dssQueue.pushQueryQueue(
                ...
                (err, res) => {
                    if (!err && res && res.ok && res.result) {
                        this.setStateSafe(
                            `devices.${circuit.dSUID}.EnergyMeterValue`,
                            res.result.meterValue / 3600 / 1000,
                        );
                    } else {
                        errorCounter++;
                    }
        ...
        !updateCounter && callback && setImmediate(() => callback(0, 0));
    }
    ...
    createLightDevice(dev, devId, callback) {
        const setLightValue = (outputType, value) => {
            ...
            if (dev.outputMode === 16 && dev.outputSwitchThreshold !== undefined) {
                this.setStateSafe(`${devId}.state`, value >= dev.outputSwitchThreshold);
                this.initialObjectValues[`${devId}.state`] = value >= dev.outputSwitchThreshold;
            } else {
                this.setStateSafe(`${devId}.state`, !!percentValue);
                this.initialObjectValues[`${devId}.state`] = !!percentValue;
            }

            if (this.stateMap[`${dev.dSUID}.brightness`]) {
                this.setStateSafe(this.stateMap[`${dev.dSUID}.brightness`], percentValue);
        ...
        this.dss.on(dev.dSUID, data => {
            ...
            this.dssQueue.queueUpdateOutputValue(
                dev,
                0,
                (brightness !== null && brightness !== undefined) || data.source.isGroup ? 'low' : 'medium',
                (err, value) => {
                    if (err) {
                        this.logOutputReadError(dev, 'brightness', err);
                        return;
                    }
                    setLightValue('brightness', value);
                },
                    ...
                    this.dssQueue.queueUpdateOutputValue(
                        ...
                        (err, value) => {
                            ...
                            if (dssConstants.outputChannelUnitRoleMap[outputType].native.nativeMax) {
                                value = Math.round(
                                    (value * 100) / dssConstants.outputChannelUnitRoleMap[outputType].native.nativeMax,
                                );
                            }
                            this.setStateSafe(dev.outputChannelList[outputType], value);
    ...
    createApartment(apartment, reachableZoneGroups, callback) {
        ...
        Object.keys(apartment.zones).forEach(zoneId => {
            ...
            if (zonesOnAFloor.has(String(zoneId)) || zone.isValid === false || zone.isPresent === false) {
                return;
            }
