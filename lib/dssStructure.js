...
class DSSStructure {
    ...
    deferSmartHomeMeterRetry(err) {
        ...
        this.adapter.log.warn(
            `Meter values could not be read completely via the Smart Home API (${errorMessage(
                err,
            )}); using the classic API and retrying the Smart Home API in ${minutes} minute${minutes === 1 ? '' : 's'}`,
        );
    }
    ...
    updateMeterData(callback) {
        ...
        if (this.smartHome) {
            this.updateMeterDataViaSmartHome().then(
                result => {
                    if (this.isAdapterStopping()) {
                    ...
                    if (result.succeeded && !result.missingCircuits.length) {
                        callback && callback(0, result.attempted ? 1 : 0);
                        return;
                    }
                    ...
                    this.updateMeterDataViaClassicApi(
                        (failed, total) => callback && callback(failed + smartFailed, total + smartTotal),
                ...
                err => {
                    ...
                    this.deferSmartHomeMeterRetry(err);
                    this.updateMeterDataViaClassicApi(
                        (failed, total) => callback && callback(failed + 1, total + 1),
