...
class SmartHomeOutputSync {
    ...
    constructor(options) {
        ...
        /** @type {Map<string, {dev: any, outputTypes: Set<string>, attempts: number}>} */
    ...
    requestDeviceSync(dev, outputTypes) {
        ...
        const entry = this.pending.get(dev.dSUID) || { dev, outputTypes: new Set(), attempts: 0 };
    ...
    scheduleFlush(delay) {
        if (this.flushTimer) {
            // A fixed window: the earliest trigger wins, later ones join the same read
            return;
        }
    ...
    async flush() {
        ...
        if (this.pending.size) {
            this.scheduleFlush(this.followUpDelay);
        }
    }
