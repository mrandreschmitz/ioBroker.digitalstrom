...
class SmartHomeOutputSync {
    ...
    applyStatus(status, flightStart) {
        ...
        for (const key of [...this.undeliverable.keys()]) {
            ...
            const value = SmartHomeOutputSync.collectOutputValues(statusEntry).get(outputType);
            if (value === undefined) {
                continue;
            }
            this.undeliverable.delete(key);
    ...
    static collectOutputValues(statusEntry) {
        ...
        // 0..100 scale of the level field is ASSUMED (like every measured status
        // value; the only live reading so far is 0, which is scale-neutral) - if a
        // powered-on socket ever shows powerLevel 1 instead of 100, this is the spot.
        if (!values.has('powerLevel') && powerStateLevel !== undefined) {
            values.set('powerLevel', powerStateLevel);
        }
