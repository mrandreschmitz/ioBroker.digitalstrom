...
/** The dSS fires several notifications per second while something is moving */
const DEFAULT_NOTIFICATION_DEBOUNCE = 400;
/** Even a continuous stream of notifications has to reach the consumer at some point */
const DEFAULT_NOTIFICATION_MAX_DELAY = 2000;
...
class DSSSmartHome extends EventEmitter {
     ...
     * @param {number} [options.notificationDebounce] ms, default 400
     * @param {number} [options.notificationMaxDelay] ms, default 2000
     */
    constructor(options) {
        super();
        ...
        this.notificationMaxDelay = this.options.notificationMaxDelay || DEFAULT_NOTIFICATION_MAX_DELAY;
    ...
    static async createApiKey(options) {
        ...
        try {
            ...
            if (options.appToken) {
                ...
                sessionToken = answer && answer.result && answer.result.token;
            }
            if (!sessionToken) {
    ...
    requestApplicationToken(sessionToken, name) {
        return new Promise((resolve, reject) => {
            ...
            const req = this.transport.request(
                ...
                res => {
                    ...
                    res.on('end', () => {
                        ...
                        if (!key) {
                            return reject(
                                new SmartHomeError('The dSS answered without a Location header, no key received'),
                            );
                        }
                        resolve(key);
                    });
            ...
            req.on('timeout', () => req.destroy(new SmartHomeError('Timeout while creating the application token')));
            req.on('error', reject);
            req.end(payload);
        });
    }
